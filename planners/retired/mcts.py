...
class MCTS:
    ...
    def get_best_child_node(node):
        if len(node.children) == 0:
            ...
            best_child_action_idx = np.argmax(list(node.N.values()))
            best_child_action = list(node.N.keys())[best_child_action_idx]
    ...
    def choose_next_node_to_descend_to(self):
        n_visits_to_each_action = list(self.s0_node.N.values())
        if len(np.unique(n_visits_to_each_action)) == 1:
            # todo descend to the one that has the highest FEASIBLE Q
            best_action = list(self.s0_node.Q.keys())[np.argmax(list(self.s0_node.Q.values()))]
        else:
            best_action = list(self.s0_node.N.keys())[np.argmax(n_visits_to_each_action)]
        best_action = list(self.s0_node.Q.keys())[np.argmax(list(self.s0_node.Q.values()))]
    ...
    def choose_action(self, curr_node):
        ...
        if len(list(curr_node.Q.values())) > 0:
            best_Q = np.max(list(curr_node.Q.values()))
            is_next_node_goal = np.all([child.is_goal_node for child in list(curr_node.children.values())])
