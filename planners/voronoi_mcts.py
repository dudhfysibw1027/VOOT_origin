...
class VoronoiMCTS(MCTS):
    def __init__(self, pick_pi, place_pi, sampling_strategy, environment):
        ...
        self.s0_node = VoronoiTreeNode(2 * self.max_rwd_to_go(0) / self.pick_diameter, x_diameter=self.pick_diameter,
                                       f_star=2.79)
    ...
    def simulate(self, curr_node, depth):
        ...
        if not curr_node.is_action_tried(action):
            if self.environment.is_pick_time():
                next_node = VoronoiTreeNode(2 * self.max_rwd_to_go(depth) / self.pick_diameter,
                                            x_diameter=self.pick_diameter,
                                            f_star=self.max_rwd_to_go(depth))
            else:
                next_node = VoronoiTreeNode(2 * self.max_rwd_to_go(depth) / self.place_diameter,
                                            x_diameter=self.place_diameter,
                                            f_star=self.max_rwd_to_go(depth))
