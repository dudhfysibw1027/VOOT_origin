...
def get_max_rwds_wrt_time(search_rwd_times):
    max_time = 15000
    n_evals = list(range(10, max_time, 1))
...
def get_color_for_algo(algo_name, color_dict):
    if algo_name in list(color_dict.keys()):
...
def plot_optimums(options):
    ...
    if options.domain == 'mdr':
        if options.p:
            sns.tsplot([0] * n_evals, list(range(options.mcts_iter)),
                       ci=95, condition='No objects remaining', color='magenta')
        else:
            sns.tsplot([4.1] * n_evals, list(range(options.mcts_iter)), ci=95,
