...
def get_optimal_epsilon_idx(result_dir):
    ...
    max_val = -np.inf
    for eps, val in zip(list(eps_to_max_vals.keys()), list(eps_to_max_vals.values())):
...
def plot_across_algorithms():
    ...
    if args.obj_fcn != 'shekel':
        sns.tsplot([0] * 2000, list(range(2000)), ci=95, condition='Optimum', color='magenta')
    else:
        if n_dim == 3:
            plt.plot(list(range(2000)), [4.78739] * 2000, linestyle='--', color=optimum_color, label='GA_2.4e4_evals')
        elif n_dim == 10:
            plt.plot(list(range(5000)), [6.04759] * 5000, linestyle='--', color=optimum_color, label='GA_2.65e5_evals')
        elif n_dim == 20:
            plt.plot(list(range(5000)), [3.93869] * 5000, linestyle='--', color=optimum_color, label='GA_8.10e5_evals')
    ...
    for algo_idx, algo in enumerate(algo_names):
        ...
        if n_samples_tested < n_samples:
            sns.tsplot(search_rwd_times, list(range(n_samples_tested)), ci=95, condition=algo_name,
                       color=color_dict[algo])
        else:
            sns.tsplot(search_rwd_times, list(range(n_samples)), ci=95, condition=algo_name,
                       color=color_dict[algo])
