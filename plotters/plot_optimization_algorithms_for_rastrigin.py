...
def plot_across_algorithms():
    ...
    if args.obj_fcn != 'shekel':
        ...
        if n_dim == 3:
            plt.plot(list(range(2000)), [4.89829] * 2000, linestyle='--', color=ga_color, label='GA_85k_evals')
        elif n_dim == 10:
            plt.plot(list(range(5000)), [8.96] * 5000, linestyle='--', color=ga_color, label='GA_473k_evals')
        elif n_dim == 20:
            plt.plot(list(range(5000)), [5.32] * 5000, linestyle='--', color=ga_color, label='GA_750k_evals')
    ...
    for algo_idx, algo in enumerate(algo_names):
        ...
        if n_samples_tested < n_samples:
            sns.tsplot(search_rwd_times, list(range(n_samples_tested)), ci=95, condition=algo.upper(), color=color_dict[color_names[algo_idx]])
        else:
            sns.tsplot(search_rwd_times, list(range(n_samples)), ci=95, condition=algo.upper(), color=color_dict[color_names[algo_idx]])
