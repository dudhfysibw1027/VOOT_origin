...
def separate(env, n=7): # Previously 4, 8
  ...
  table_names = [name for name in [get_name(body) for body in env.GetBodies() if not body.IsRobot()] if 'table' in name]
...
def dantam(env): # (Incremental Task and Motion Planning: A Constraint-Based Approach)
  ...
  pose_indices = list(product(list(range(m)), list(range(n))))
  colors = {}
  ...
  return ManipulationProblem(function_name(inspect.stack()),
    object_names=list(initial_poses.keys()), table_names=[get_name(table)],
    goal_poses=goal_poses,
    initial_poses=initial_poses, known_poses=list(poses.values()))
