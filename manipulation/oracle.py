...
class ManipulationOracle(object):
  ...
  def setup_robot(self, debug, active_arms=None, reset=True):
    ...
    self.robot_link_meshes = {name: link_mesh(self, link) for name, link in self.robot_links.items()}
    ...
    for name, link_names in self.robot_parts.items():
  ...
  def setup_bodies(self):
    ...
    self.body_name_to_geom_hash = {body_name: geometry_hash(body) for body_name, body in list(self.bodies.items())}
    self.geom_hash_to_body_name = {geometry_hash(body): body_name for body_name, body in list(self.bodies.items())}
    ...
    self.obstacles = [name for name, body in list(self.bodies.items()) if name not in self.get_objects() and not body.IsRobot()]
    self.initial_poses = {object_name: self.get_pose(object_name) for object_name in self.get_objects()}

  def setup_regions(self):
    floors = [AASurface('floor', list(zip(self.env_min, self.env_max))[:2], BODY_Z_OFFSET, color=FLOOR_COLOR)] # TODO - is it correct to use env_min/env_max for placements?
  ...
  def setup_bounding_volumes(self):
    self.radii2D = {name: get_radius2D(body)**2 for name, body in self.bodies.items()}
    self.radii = {name: get_radius(body)**2 for name, body in self.bodies.items()}
    self.aabbs = {}
    self.meshes = {}
    for name, body in self.bodies.items(): # TODO - use geom_hash instead of object names
  ...
  def setup_caches(self):
    ...
    self.caches = [value for value in list(self.__dict__.values()) if isinstance(value, DeterminisiticCache)]
  ...
  def draw_goals(self):
    self.goal_handles = []
    for object_name, pose in list(self.problem.goal_poses.items()):
      self.goal_handles.append(draw_point(self.env, point_from_pose(pose.value), get_color(self.get_body(object_name))))
    for object_name, (surface, point) in list(self.problem.goal_constrained.items()):
