...
class GraspOptions(object):
  ...
  def options(self):
    return {name: value for name, value in list(self.__dict__.items()) if name != '__hash'}
  ...
  def __hash__(self):
    if '__hash' not in self.__dict__: self.__dict__['__hash'] = hash(frozenset(list(self.options().items())))
...
class OpenRaveSideGraspOptions(SideGraspOptions):
  ...
  def grasp_trans(self, oracle):
    ...
    horizontal_grasps = [g for g in g_model.grasps if abs(g_model.getGlobalApproachDir(g)[2]) < 0.01]
    ...
    return [g_model.GetLocalGraspTransform(grasp, collisionfree=False) for grasp in g_model.grasps]  # compute_grasp(g_model.getGlobalGraspTransform(grasp, collisionfree=False), get_trans(body))
