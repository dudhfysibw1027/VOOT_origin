...
class Trajectory(object):
  ...
  def samples(self, time_step=0.1):
    return list(map(self.config, get_samples(self.traj(), time_step=time_step)))
  ...
  def waypoints(self):
    return list(map(self.config, get_waypoints(self.traj())))
  def step(self, sequence=None):
    ...
    with self.cspace.body:
      ...
      for i, config in enumerate(sequence):
        ...
        input('Finish' if i == len(sequence) - 1 else 'Step %d'%(i+1))
  def execute(self):
