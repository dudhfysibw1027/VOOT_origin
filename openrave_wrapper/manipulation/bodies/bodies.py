...
def place_body(env, body, xxx_todo_changeme, base_name): # TODO - make it place on the relative table
  (x, y, theta) = xxx_todo_changeme
...
def place_body(env, body, xxx_todo_changeme1, base_name): # TODO - make it place on the relative table
  (x, y, theta) = xxx_todo_changeme1
  body_aabb = aabb_from_body(body)
  base_aabb = aabb_from_body(env.GetKinBody(base_name))
  z = get_point(body)[2] - (body_aabb.pos()[2] - body_aabb.extents()[2]) + \
      (base_aabb.pos()[2] + base_aabb.extents()[2]) + BODY_PLACEMENT_Z_OFFSET
...
def place_xyz_body(env, body, xxx_todo_changeme2, base_name): # TODO - make it place on the relative table
  (x, y, z, theta) = xxx_todo_changeme2
  body_aabb = aabb_from_body(body)
  base_aabb = aabb_from_body(env.GetKinBody(base_name))
  z = get_point(body)[2] - (body_aabb.pos()[2] - body_aabb.extents()[2]) + \
...
def place_body_on_floor(body, xxx_todo_changeme3):
  (x, y, theta) = xxx_todo_changeme3
