...
def execute_plan(plan, oracle, pause=True):
  ...
  def execute():
    ...
    input('Hit enter to finish' if len(plan.operators) == 0 else 'Hit enter to start')
    for i, action in enumerate(plan.operators):
      if isinstance(action, Executable):
        action.execute(oracle)
      if i == len(plan.operators) - 1: input('Hit enter to finish')
      elif pause: input('Hit enter to continue')
...
def visualize_plan(plan, oracle, display=True, save=None):
  ...
  def execute():
    ...
    update_viewer()
    if display: input('Hit enter to finish' if len(plan.operators) == 0 else 'Hit enter to step')
    ...
    for i, action in enumerate(plan.operators):
      if isinstance(action, StepExecutable):
        for j, _ in enumerate(action.step(oracle)):
          update_viewer()
          if display: input('Hit enter to finish' if i == len(plan.operators) - 1 else 'Hit enter to step')
...
def visualize_states(states, oracle, display=True, save=None):
  ...
  def execute():
    ...
    for i, state in enumerate(states):
      ...
      if display: input('Hit enter to finish' if i == len(states) - 1 else 'Hit enter to step')
      else: sleep(1) # Gives time for the viewer to update
