  ...
  def generator(paps):
    ...
    for pap in paps:
      if (ONLY_HOLDING_COLLISIONS and not any(oracle.holding_collision(pap.grasp_config,
            o, p, Holding(body_name, pap.grasp)) for o, p in object_poses.items())) or \
         (not ONLY_HOLDING_COLLISIONS and not any(oracle.traj_holding_collision(pap.approach_config,
          pap.trajs, o, p, Holding(body_name, pap.grasp)) for o, p in object_poses.items())):
...
def generate_paps(oracle, body_name, poses=[], num_poses=None, grasps=[], num_grasps=None, regions=None, stackings=None,
  ...
  poses = sorted([p for p in poses if not any(oracle.object_collision(body_name, p, o, p)
      for o, p in object_poses.items())], key=pose_sort_fn)
  ...
  def generator(paps):
    for pap in paps:
      if (oracle.approach_collision(body_name, pap.pose, pap.grasp) or (not ONLY_HOLDING_COLLISIONS and \
          any(oracle.obj_approach_collision(body_name, pap.pose, pap.grasp, o, p) for o, p in object_poses.items()))): continue
      with oracle.state_saver():
