...
def query_from_edge_push_trajs(oracle, body_name, region_name, goal_point):
  geom_hash = oracle.get_geom_hash(body_name)
  for pose in [p for p in oracle.pushes[oracle.get_geom_hash(body_name)] if oracle.on_edge(region_name, body_name, p)]:
...
def query_to_edge_push_trajs(oracle, body_name, region_name, start_pose, grasps):
  ...
  for pose in [p for p in oracle.pushes[oracle.get_geom_hash(body_name)] if oracle.on_edge(region_name, body_name, p) and \
