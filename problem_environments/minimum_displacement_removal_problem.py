...
def load_objects(env, obj_shapes, obj_poses, color):
    ...
    OBJECTS = []
    i = 0
    nobj = len(list(obj_shapes.keys()))
    for obj_name in list(obj_shapes.keys()):
...
def generate_poses_and_place_shelf_objs(objects, regions, env):
    ...
    obj_poses = {obj.GetName(): get_pose(obj) for obj_list in list(objects.values()) for obj in obj_list}
    return obj_poses
...
class MinimumDisplacementRemovalProblem:
    ...
    def set_obj_poses(self, problem_idx):
        ...
        for obj_name, obj_pose in zip(list(obj_poses.keys()), list(obj_poses.values())):
