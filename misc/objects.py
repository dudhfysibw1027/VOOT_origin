from collections.abc import namedtuple, defaultdict, Counter as Mapping, Iterable, Iterator, Set
from collections import deque, defaultdict, Counter as CountDict, Iterable, Iterator, Set
...
def str_object(obj):
  ...
  if type(obj) == dict:
    return '{%s}' % ', '.join(str_object(item) + ': ' +  str_object(obj[item]) \
        for item in sorted(list(obj.keys()), key=lambda k: str_object(k)))
...
def str_args(args):
  return '(%s)' % ', '.join([str_object(item) + '=' +  str_object(args[item]) \
      for item in sorted(list(args.keys()), key=lambda k: str_object(k))])
...
class HashObject(object): # TODO - related to namedtuple
  ...
  def __hash__(self):
    if '__hash' not in self.__dict__:
      self.__dict__['__hash'] = hash(frozenset(list(self.__dict__['__hash_dict'].items())))
...
class SinglePrintObject(HashObject):
  def __str__(self):
    ...
    return class_name(self) + str_object(list(self.__dict__['__hash_dict'].values())[0])
  __repr__ = __str__
...
class FrozenDict(Mapping):
  ...
  def __hash__(self):
    if self._hash is None:
      self._hash = 0
      for pair in self.items(): self._hash ^= hash(pair)
    return self._hash
